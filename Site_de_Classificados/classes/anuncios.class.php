...
class Anuncios {
        ...
        if (count($fotos) > 0) {
            for ($q = 0; $q < count($fotos["tmp_name"]); $q++) {
                ...
                if (in_array($tipo, array("image/jpeg", "image/png"))) {
                    // MOVENDO A IMAGEM PARA SUA PASTA
    ...
    public function excluirFoto($id) {
        global $pdo;
        $sql = $pdo->prepare("SELECT id FROM anuncios_imagens WHERE id = :id");
        $sql->bindValue(":id", $id);
        $sql->execute();
        $id_anuncio = 0;
        if ($sql->rowCount() > 0) {
    ...
    public function getUltimosAnuncios($pagina, $porPagina, $filtros) {
        global $pdo;
        // SE PAGINA FOR 1 = 1 - 1 = 0
        // 2 = QUANTIDADE DE ITENS QUE SERÃO PEGADOS
        ...
        FROM anuncios WHERE " . implode(' AND ', $filtrostring) . "
        ORDER BY id DESC
